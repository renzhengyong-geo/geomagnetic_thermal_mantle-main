...
double Yoshino_garnet_conductivity(double temperature, double water_content, double pressure) {
    // Constants
    ...
    const double sigma_h = 1070;              // Hydrogen-related conductivity in S/m
    const double Hh = 1.59;                   // Activation enthalpy for hydrogen in eV
...
int main() {
    ...
    try {
        double conductivity = Yoshino_garnet_conductivity(temperature, water_content, pressure);
