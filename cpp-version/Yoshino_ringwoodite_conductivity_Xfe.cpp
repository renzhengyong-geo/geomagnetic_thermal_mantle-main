...
double Yoshino_ringwoodite_conductivity_Xfe(double temperature, double water_content, double pressure, double Xfe) {
    // Constants
    ...
    const double sigma_h = 1885;              // Hydrogen-related conductivity in S/m
    const double Hh = 1.93;                   // Activation enthalpy for hydrogen in eV
    const double alpha_h = 1.46;              // Constant for hydrogen conduction
    const double V0 = -0.33;                  // Activation volume
    const double belta_h = 0.72;              // Beta constant for hydrogen
    const double sigma_p = 27.7;              // Proton-related conductivity in S/m
    const double Hp = 1.12;                   // Activation enthalpy for proton conduction in eV
    const double alpha = 0.67;                // Constant for proton conduction
...
int main() {
    ...
    try {
        double conductivity = Yoshino_ringwoodite_conductivity_Xfe(temperature, water_content, pressure, Xfe);
