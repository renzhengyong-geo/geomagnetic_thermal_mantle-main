...
double Yoshino_Perovskite_conductivity(double temperature, double water_content, double pressure) {
    // Constants
    ...
    const double sigma_i = 81;                // Intrinsic conductivity in S/m
    const double Hi = 0.70 + 0.03;            // Activation enthalpy for intrinsic conduction in eV
...
int main() {
    ...
    try {
        double conductivity = Yoshino_Perovskite_conductivity(temperature, water_content, pressure);
