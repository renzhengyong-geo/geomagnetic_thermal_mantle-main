...
double Yoshino_C2c_conductivity(double temperature, double water_content, double pressure) {
    // Constants
    ...
    const double sigma_i = std::pow(10, 3.9); // Intrinsic conductivity in S/m
...
int main() {
    ...
    try {
        double conductivity = Yoshino_C2c_conductivity(temperature, water_content, pressure);
