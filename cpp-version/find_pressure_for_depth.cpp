...
// Linear interpolation function, defined outside of findPressureForDepth
double linearInterpolate(double x0, double y0, double x1, double y1, double x) {
    return y0 + (y1 - y0) * ((x - x0) / (x1 - x0));
}

// Function to find the pressure associated with a given depth using binary search and interpolation
double findPressureForDepth(double inputDepth) {
    // Define the data points (depth and pressure, km and Gpa)
    // Depth-pressure data from 0 km to 45 km is linearly interpolated using the PREM model.
    // Reference: Dziewonski, Adam M.; Anderson, Don L. (June 1981). "Preliminary reference Earth model" (PDF).
    // Physics of the Earth and Planetary Interiors, 25 (4): 297–356. Bibcode:1981PEPI...25..297D.
    // doi:10.1016/0031-9201(81)90046-7.
    // Depth-pressure data 50 km to 2600 km from Katsura, T. (2022). A Revised Adiabatic Temperature Profile for the Mantle. 
    // Journal of Geophysical Research: Solid Earth, 127(2), e2021JB023562.
    // https://doi.org/10.1029/2021JB023562
    ...
    if (inputDepth <= data.front().first) {
        return data.front().second;
    }

    // If inputDepth is greater than or equal to the largest depth, return the largest pressure
    if (inputDepth >= data.back().first) {
        return data.back().second;
    }

    // Use lower_bound to find the first element that is not less than the inputDepth
    std::vector<std::pair<double, double>>::iterator it = std::lower_bound(data.begin(), data.end(), std::make_pair(inputDepth, 0.0));

    // Perform linear interpolation between the found point and the previous point
    std::vector<std::pair<double, double>>::iterator prev = std::prev(it);
    return linearInterpolate(prev->first, prev->second, it->first, it->second, inputDepth);
}

/*
int main() {
    // Input the depth value from the user
    double inputDepth;
    std::cout << "Enter the depth (in km): ";
    std::cin >> inputDepth;

    // Find the corresponding pressure using interpolation if necessary
    double pressure = findPressureForDepth(inputDepth);
    std::cout << "The pressure at depth " << inputDepth << " km is: " << pressure << " GPa" << std::endl;
