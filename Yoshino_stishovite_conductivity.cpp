...
double Yoshino_stishovite_conductivity(double temperature, double water_content, double pressure) {
    // Constants
    ...
    const double sigma_i = 137;               // Intrinsic conductivity in S/m
    ...
    const double sigma_p = 1042;              // Proton-related conductivity in S/m
    const double Hp = 1.07;                   // Activation enthalpy for proton conduction in eV
    const double alpha = 0.19;                // Constant for proton conduction
...
int main() {
    ...
    try {
        double conductivity = Yoshino_stishovite_conductivity(temperature, water_content, pressure);
