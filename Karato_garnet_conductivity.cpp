...
// Function to compute the conductivity of Garnet using Karato's model
double Karato_garnet_conductivity(double T, double Cw, double P) {
    // Constants for the equation
    const double A1 = pow(10, 2.1);   // Pre-exponential factor for polaron conduction, S/m
    const double A2 = pow(10, 2.7);   // Pre-exponential factor for proton conduction, S/m
    const double r1 = 0;              // Empirical constant for polaron conduction
    const double r2 = 0.63;           // Empirical constant for proton conduction
    const double E1 = 128000;         // Activation energy for polaron conduction, J/mol
    const double E2 = 70000;          // Activation energy for proton conduction, J/mol
    const double V1 = 2.5;            // Activation volume for polaron conduction, cc/mol
    const double V2 = -0.6;           // Activation volume for proton conduction, cc/mol
...
int main() {
    ...
    double sigma = Karato_garnet_conductivity(T, Cw, P);
