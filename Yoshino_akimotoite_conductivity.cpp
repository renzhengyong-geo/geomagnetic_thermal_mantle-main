...
double Yoshino_akimotoite_conductivity(double temperature, double water_content, double pressure) {
    // Constants
    ...
    const double sigma_i = 15 - 1;            // Intrinsic conductivity in S/m
    const double Va = -1.5;                   // Activation volume in some units
    const double Hi = (0.82 - 0.01) + pressure * Va * 0.0104;  // Activation enthalpy in eV
    ...
    const double r = 0;                       // Empirical constant for proton conduction
    ...
    double sigma = sigma_i * std::exp(-Hi / (k * temperature)) +
                   ...
                   sigma_p * std::pow(water_content, r) * std::exp(-Hp / (k * temperature));
...
int main() {
    ...
    try {
        double conductivity = Yoshino_akimotoite_conductivity(temperature, water_content, pressure);
