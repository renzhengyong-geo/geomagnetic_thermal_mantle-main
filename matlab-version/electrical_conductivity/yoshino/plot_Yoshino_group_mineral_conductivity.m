...
% Input Parameters
group_id = "YoSHINO";
fileName = "Yoshino_AREPS_2013_pyrolite.tab"; % for fig 7
...
m = size(upperMantleTable, 1); % Number of rows
water_content = ones(m, 1);    % Water content (wt%)
[~, ~, um_sigma_plus_1, um_sigma_minus_1, ~] = compute_upper_mantle_conductivity_depth_profile(upperMantleTable, water_content, group_id);
[~, ~, um_sigma_plus_01, um_sigma_minus_01, ~] = compute_upper_mantle_conductivity_depth_profile(upperMantleTable, water_content * 0.1, group_id);
[~, ~, um_sigma_plus_001, um_sigma_minus_001, ~] = compute_upper_mantle_conductivity_depth_profile(upperMantleTable, water_content * 0.01, group_id);
[~, ~, um_sigma_plus_0001, um_sigma_minus_0001,~] = compute_upper_mantle_conductivity_depth_profile(upperMantleTable, water_content * 1e-5, group_id);
% Compute conductivity-depth profile in the transition zone
m = size(upperTransitionZoneTable, 1); % Number of rows
water_content = ones(m, 1);    % Water content (wt%)
[~, ~, ut_sigma_plus_1, ut_sigma_minus_1, ~] = compute_upper_transition_zone_conductivity_depth_profile(upperTransitionZoneTable, water_content, group_id);
[~, ~, ut_sigma_plus_01, ut_sigma_minus_01,~] = compute_upper_transition_zone_conductivity_depth_profile(upperTransitionZoneTable, water_content * 0.1, group_id);
[~, ~, ut_sigma_plus_001, ut_sigma_minus_001,~] = compute_upper_transition_zone_conductivity_depth_profile(upperTransitionZoneTable, water_content * 0.01, group_id);
[~, ~, ut_sigma_plus_0001, ut_sigma_minus_0001, ~] = compute_upper_transition_zone_conductivity_depth_profile(upperTransitionZoneTable, water_content * 1e-5, group_id);
m = size(lowerTransitionZoneTable, 1); % Number of rows
water_content = ones(m, 1);    % Water content (wt%)
[~, ~, lt_sigma_plus_1, lt_sigma_minus_1, ~] = compute_lower_transition_zone_conductivity_depth_profile(lowerTransitionZoneTable, water_content, group_id);
[~, ~, lt_sigma_plus_01, lt_sigma_minus_01,~] = compute_lower_transition_zone_conductivity_depth_profile(lowerTransitionZoneTable, water_content * 0.1, group_id);
[~, ~, lt_sigma_plus_001, lt_sigma_minus_001,~] = compute_lower_transition_zone_conductivity_depth_profile(lowerTransitionZoneTable, water_content * 0.01, group_id);
[~, ~, lt_sigma_plus_0001, lt_sigma_minus_0001, ~] = compute_lower_transition_zone_conductivity_depth_profile(lowerTransitionZoneTable, water_content * 1e-5, group_id);
...
% Input Parameters
group_id = "YoSHINO";
fileName = "Yoshino_AREPS_2013_pyrolite_300k_lower.tab"; % for fig 7
