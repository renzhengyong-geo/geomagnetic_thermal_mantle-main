function depth = find_depth_for_pressure(inputPressure)
    % find_depth_for_pressure Computes depth corresponding to a given pressure using interpolation.
    %
    % INPUT:
    %   inputPressure - Pressure in GPa.
    %
    % OUTPUT:
    %   depth - Depth in km corresponding to the input pressure.
    %
    % This function assumes a predefined depth-pressure relationship based on the PREM model.
    ...
    % Define the depth-pressure data (depth in km and pressure in GPa)
    data = [
    ...
    % Ensure inputPressure is within the valid range
    if inputPressure <= data(1, 2)
        depth = data(1, 1);
        return;
    end

    if inputPressure >= data(end, 2)
        depth = data(end, 1);
        return;
    end

    % Find index of lower and upper bounds
    idx = find(data(:, 2) >= inputPressure, 1);

    % Linear interpolation
    depth = linearInterpolate(data(idx - 1, 2), data(idx - 1, 1), data(idx, 2), data(idx, 1), inputPressure);
end

% Helper function for linear interpolation
function value = linearInterpolate(x1, y1, x2, y2, x)
