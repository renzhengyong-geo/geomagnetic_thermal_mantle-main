...
function sigma = Yoshino_ferropericlase_conductivity(temperature, water_content,pressure)
    % Constants
...
%     pressure = 1e3 * pressure;  % 1 Pa = 1 J/m³ = 1e-6 J/cc
    ...
    sigma = sigma_i*exp(-Hi/(k*T))+...
        sigma_h*Xfe*exp(-((Hh-alpha*Xfe^(1.0/3.0)+0.0104*pressure*(V0-belta*Xfe^(1.0/3.0))))/(k*T))+...
