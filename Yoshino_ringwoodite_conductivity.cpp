...
// Function to compute the conductivity of Ringwoodite using Yoshino's model
double Yoshino_ringwoodite_conductivity(double T, double Cw, double Xfe) {
    ...
    const double sigma_0h = pow(10, 2.92);    // S/m (Pre-exponential factor for hopping conduction)
    const double delta_Hh = 1.36 * 1.60218e-19; // J (Converted from eV to J)
    const double sigma_0p = pow(10, 1.44);    // S/m (Pre-exponential factor for proton conduction)
    const double delta_H0_p = 1.12 * 1.60218e-19; // J (Converted from eV to J)
    const double alpha_p = 0.67 * 1.60218e-19; // J (Converted from eV to J)
...
int main() {
    ...
    double sigma = Yoshino_ringwoodite_conductivity(T, Cw, Xfe);
