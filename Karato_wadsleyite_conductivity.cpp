...
// Function to compute the conductivity of Wadsleyite using Karato's model
double Karato_wadsleyite_conductivity(double T, double Cw, double P) {
    // Constants for the equation
    const double A1 = pow(10, 2.1);    // Pre-exponential factor for polaron conduction, S/m
    const double A2 = pow(10, 2.1);    // Pre-exponential factor for proton conduction, S/m
    const double r1 = 0;               // Empirical constant for polaron conduction
    const double r2 = 0.72;            // Empirical constant for proton conduction
    const double E1 = 147000;          // Activation energy for polaron conduction, J/mol
    const double E2 = 88000;           // Activation energy for proton conduction, J/mol
...
int main() {
    ...
    double sigma = Karato_wadsleyite_conductivity(T, Cw, P);
