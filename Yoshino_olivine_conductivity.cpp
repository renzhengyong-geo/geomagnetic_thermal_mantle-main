...
// Function to compute the conductivity of olivine using Yoshino's model
double Yoshino_olivine_conductivity(double T, double Cw, double Xfe) {
    // Constants for the equation
    const double sigma_0i = pow(10, 4.73);   // S/m (Pre-exponential factor for ionic conduction)
    const double delta_Hi = 2.31 * 1.60218e-19;  // J (Converted from eV to J)
    const double sigma_0h = pow(10, 2.98);   // S/m (Pre-exponential factor for hopping conduction)
    const double delta_Hh = 1.71 * 1.60218e-19;  // J (Converted from eV to J)
    const double sigma_0p = pow(10, 1.90);   // S/m (Pre-exponential factor for proton conduction)
    const double delta_H0_p = 0.92 * 1.60218e-19; // J (Converted from eV to J)
    const double alpha_p = 0.16 * 1.60218e-19;    // J (Converted from eV to J)
...
int main() {
    ...
    double sigma = Yoshino_olivine_conductivity(T, Cw, Xfe);
